...
namespace Microsoft.AspNetCore.Blazor.Hosting
{
    ...
    /// </summary>
    /// <remarks>
    /// Calling <see cref="RunAsync"/> will result in disposal of the host object. If the host is going
    /// to used to run an application, then it is not necessary to manually dispose the host.
    /// </remarks>
    public sealed class WebAssemblyHost : IDisposable, IAsyncDisposable
    {
        ...
        //
        // However, the way that users primarily interact with the host is by calling RunAsync().
        // Hosts are single-use, and once RunAsync() is called, it can't be called again for the same object.
        // Therefore we dispose the host inside RunAsync().
        //
        // We need to make sure the host is safe for double-dispose in case users do it. The behavior is
        // surprising anyway, so we want to make sure it's not an error.
        //
        // The twist is that in general dispose is not going to run. When a user refreshes or navigates away
        // that terminates the app, like a process.exit. So the dispose functionality here is basically so that
        // it can be used in unit tests. Based on the APIs that exist in Blazor today it's not possible for the
